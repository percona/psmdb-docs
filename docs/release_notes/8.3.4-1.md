...
- [SERVER-79980 octicons-link-external-16:](https://jira.mongodb.org/browse/SERVER-79980){="_blank"}: Added TLS 1.3 support for MongoDB Server on Windows for both incoming and outgoing connections.

- [SERVER-83322 octicons-link-external-16:](https://jira.mongodb.org/browse/SERVER-83322){="_blank"}: Improved query planning for compound wildcard indexes. Filters on fields outside the index no longer cause the bounds for indexed fields to revert to [`MinKey`, `MaxKey`], allowing queries to use more selective index bounds.

- [SERVER-108215 octicons-link-external-16:](https://jira.mongodb.org/browse/SERVER-108215){="_blank"}: Improved transaction handling by limiting the number of multi-document transactions that can remain open at the same time. This prevents large numbers of idle transactions from consuming resources and affecting server performance. When the limit is reached, MongoDB rejects new transactions with an error.

- [SERVER-127346 octicons-link-external-16:](https://jira.mongodb.org/browse/SERVER-127346){="_blank"}: Improved session refresh and cleanup during heavy workloads. These tasks now bypass busy queues, preventing delays and errors.
...
-[SERVER-129103](https://jira.mongodb.org/browse/SERVER-129103){:target="_blank"} ([CVE-2026-13077 :octicons-link-external-16:](https://www.cve.org/CVERecord?id=CVE-2026-13077){:target="_blank"}):
