...
-[SERVER-128387](https://jira.mongodb.org/browse/SERVER-128387){:target="_blank"} ([CVE-2026-13070 :octicons-link-external-16:](https://www.cve.org/CVERecord?id=CVE-2026-13070){:target="_blank"}): When using PROXY protocol v2 on a Unix domain socket path, roles derived from X.509 client certificates may not be validated against the configured list of trusted certificate authorities (`tlsCATrusts`). This can lead to unintended role assignments during MONGODB-X509 authentication.
...
-[SERVER-128362](https://jira.mongodb.org/browse/SERVER-128362){:target="_blank"} ([CVE-2026-13070 :octicons-link-external-16:](https://www.cve.org/CVERecord?id=CVE-2026-13070){:target="_blank"}): A MongoDB server initiating an outbound TLS connection may terminate abnormally when processing a malformed OCSP response from a remote peer during the TLS handshake. OCSP stapling validation is enabled by default for outgoing TLS connections. Affected scenarios require the remote peer to hold a certificate issued by the cluster's trusted certificate authority, or for the connection to traverse an untrusted network path. 
