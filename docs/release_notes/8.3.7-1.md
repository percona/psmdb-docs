...
#### Expanded aggregation capabilities
...
#### Aggregation and data conversion 
...
FTDC collects connection pool statistics for `mongod`. The `serverStatus` output includes additional metrics for query planning, replication, TTL processing, search operations, and execution queues.
...
#### Security
Starting with MongoDB 8.3, you cannot include SCRAM-SHA-1 for `authenticationMechanisms` while also specifying `mongod --tlsFIPSMode` or `mongos --tlsFIPSMode`.

Before upgrading, review your authentication configuration and remove SCRAM-SHA-1 if FIPS mode is enabled. If both settings are present, the server rejects the configuration and logs the following message:
