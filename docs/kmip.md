...
???+ admonition "Version changes"
     ...
     |Version         | Description |
     |----------------|-------------|
     | [6.0.17-14](release_notes/6.0.17-14.md)| [Key state polling](#key-state-polling).|
...
Percona Server for MongoDB supports [master key rotation](https://www.mongodb.com/docs/manual/tutorial/rotate-encryption-key/#kmip-master-key-rotation). This enables users to comply with data security regulations when using KMIP.
...
| Configuration file  | {{optionlink('security.kmip.useLegacyProtocol')}}|
|-------------------- | --------------------|
