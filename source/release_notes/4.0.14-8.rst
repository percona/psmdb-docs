...
|PSMDB| |release| is based on `MongoDB 4.0.14
<https://docs.mongodb.com/manual/release-notes/4.0/#dec-18-2019>`_.

Bugs Fixed
================================================================================

- :psmdb-bug:`473`: The ``logApplicationMessage`` command failed even
  when it was run by the user with extended privileges. The problem
  has been fixed to allow running the ``logApplicationMessage`` command by
  any role that has the ``applicationMessage`` privilege, such as
  `clusterManager` or `hostManager`.

  .. code-block:: guess

     > db.runCommand({logApplicationMessage: 'find'});

.. |release| replace:: 4.0.14-8
.. |date| replace:: January 9, 2020
