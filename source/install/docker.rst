...
To run |PSMDB| 3.0 in a Docker container, use the following command::

 docker run -d \
  --name psmdb \
  --restart always \
  percona/percona-server-mongodb:3.0
...
* ``percona/percona-server-mongodb:3.0`` is the name and version tag
