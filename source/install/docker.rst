...
To run |PSMDB| 3.2 in a Docker container, use the following command::

 docker run -d \
  --name psmdb \
  --restart always \
  percona/percona-server-mongodb:3.2
...
* ``percona/percona-server-mongodb:3.2`` is the name and version tag
